...
var Haiku = Origami.extend({
  ...
  read: function(){
    ...
    haiku._read(sourcedir, function(){
      ...
      _.each(haiku.collections, function(keys, collection){
        ...
        haiku.set(attrs);
        // console.log('collection'.yellow, collection);
        // console.log('keys'.yellow, keys);
      });

      // once all the collections are set we need to set the content attribute
      // on each piece of content
      // _.each(haiku.content)
  ...
  addContent: function(content){
    ...
    if (content.isInCollection()) {
      ...
      if (_.include(_.keys(haiku.collections), collection)){
        haiku.collections[collection].push(content.url());
        // haiku.get(collection).push(content);
