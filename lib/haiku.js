...
var Haiku = Origami.extend({
  ...
  read: function(){
    ...
    // getting partials
    var haiku = this;
    var partialFiles = fs.readdirSync(this.config.templatesdir());

    var partials = {};

    _.each(partialFiles, function(pathname){
      var fullpathname = path.join(haiku.config.templatesdir(), pathname);
      var stats = fs.statSync(fullpathname);

      if (stats.isFile()){
        var name = path.basename(pathname, '.mustache');
        partials[name] = fs.readFileSync(fullpathname, 'utf8')
      }
    });

    var layouts = {};
    var layoutsdir = path.join(this.config.templatesdir(), 'layouts');
    var layoutFiles = fs.readdirSync(layoutsdir);

    _.each(layoutFiles, function(pathname){
      var fullpathname = path.join(layoutsdir, pathname);
      var stats = fs.statSync(fullpathname);

      if (stats.isFile()){
        var name = path.basename(pathname, '.mustache');
        layouts[name] = fs.readFileSync(fullpathname, 'utf8')
      }
    });

    haiku.partials = partials;
    haiku.layouts = layouts;
