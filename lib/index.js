...
Haiku.prototype.render = function(key, context, callback) {
  ...
  haiku.get(key, function(err, json) {
    ...
    haiku.template.render(page.url, context, function(err, content) {
      if (err) return callback(err)
      ...
      var layout = context.layout || page.layout
      var normalized = 'layouts/' + layout + '.mustache'

      context['layout-content'] = content

      haiku
      .template
      .render(normalized, context, callback)
    })
  })
}
