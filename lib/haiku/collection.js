...
var Collection = function(options) {
  ...
  // "content" and we also have a class Content
...
Collection.prototype = {
  ...
  _buildIndex: function() {
    var collection = this // for callbacks
      ...
      // anything with a getIndex function defined is a collection; probably should
      // encapsulate that or use instanceof but this is an internal function anyway
      , collections = _(collection.folder).select(function(entry,name) { return entry.getIndex; })
      
      // the entries are non-collections, that is, everything else; we just use reject
      // instead of select
      , entries = _(collection.folder).reject(function(entry,name) { return entry.getIndex; })
      
      // we build up the initial index just using the entries, since they're simple
      , index = _(entries).reduce(function(index,entry) { index[entry.name] = entry; return index; }, {})
    ;
    
    // next we have to get the indices for each collection and add them into our index, only
    // with the name of collection prefixed to it; that's how we get "foo/bar" in there
