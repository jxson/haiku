...
//
// The path option really should be an absolute path
...
_.extend(Collection.prototype, {
  ...
  cssID: function(){
    var _path = this.options.path
      , root = this.site.directories.content
      , relativePath = _path.replace(root, '')
    ;

    return  'collection' + relativePath.replace(/\//g, '-')
  },
  ...
  read: function(){
    ...
    fs.readdir(collection.path, function(err, list){
      ...
      // If the list is empty trigger the collection's ready event and don't
      // bother continuing.
      if (_.isEmpty(list)) return collection.emit('ready');
  ...
  pages: function() {
    ...
    selection = items.select(function(entry, name){
      if (entry.attributes && entry.attributes.published) sortByDate = true;

      return entry instanceof Page;
    })

    if (sortByDate){
      selection = _.sortBy(selection, function(item){
        if (item.attributes && item.attributes.published){
          return item.attributes.published;
        }
      });
    }
  ...
  toJSON: function() {
    return _.extend({}, this.attributes, {
      pages: _(this.pages()).reduce(function(array, page) {
        ...
        // Make sure that index files don't get into the array
        if (! page.route().match(page.site.options.index)){
          array.push(page.toJSON());
        }
