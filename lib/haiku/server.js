...
var Server = function(attrs){
  ...
  server.app.get('/', function(req, res){
    ...
    // console.log('index'.magenta, index);

    if (index){
      index.render(function(err, html){
...
Server.prototype.run = function(){
  ...
  server.haiku.on('ready', function(){
    ...
    // var bad = server.haiku.content['/blog/atom.xml.xml']
    // var path = require('path')
    // console.log('\n BAD SEED! \n'.bold.red.inverse, bad.buildpath());
    // console.log(bad.get('file').replace(/(.*)\/content\//g, ''));
    // console.log(path.extname(bad.get('file')));
    // console.log(bad._extension());
