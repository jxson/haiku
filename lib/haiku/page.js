...
Page.processors = {
  ...
  mustache: function(content, attributes, site) {
    try {
      ...
      if (e.message.match(/unknown_partial/)){
        var message = 'Mustache bugged out - ' + e.message + '\n' +
              '  (make sure to add the partial to your templates directory)'
        ;
      }
...
_.extend(Page.prototype, {
  ...
  // # page.route()
  //
  // Get the route for this page, useful for the keys in the indexes, the
  // page.url() can contain some stuff that will confuse the server when
  // looking up content
  route: function(){
    if (! this.buildPath()){ return; }
  ...
  _processor: function(data, attributes, site){
    var parser = this.parser()
      , templateEngine = this.site.options.defaultTemplateEngine
      , preprocessed = this._preprocess(data, attributes)
    ;
  ...
  _preprocess: function(data, attributes){
    var page = this
      , site = this.site
    ...
    // TODO: clean this up
    // Allows the defaultTemplateEngine to be set on the parent for controlling whole
    if (page.parent && parent.options){
      defaultTemplateEngine = parent.options.defaultTemplateEngine
    } else {
      defaultTemplateEngine = 'mustache'
    }

    // the default template engine had been disabled, check for an explicitly
    // set engine in the filename.
    if (defaultTemplateEngine === false) {
      var chunks = page.basename().split('.')
        , engines = ['mustache'] // TODO: move this into a static object
      ;

      // Check that this page is requesting a particular engine
      _.each(engines, function(engine){
        templateEngine = _.include(chunks, engine) ? engine : false;
      });
    } else {
    ...
    if (templateEngine) {
      return Page.processors[templateEngine](data, attributes, site)
    } else {
  ...
  render: function(attributes){
    var page = this
      ...
      , json = page.toJSON()
      , log = this.logger
      , isTemplate = page.path.match(site.directories.templates)
      , layout
      , content
    ;
    ...
    // If there is a layout get it. Also no layouts for templates
    if (attributes.layout && attributes.layout !== '' && !isTemplate){
      // Make sure the layouts collection exists
      if (! page.site.layouts){
        var message = [
          'WOAH!'.bold,
          '',
          'haiku can not find your layouts directory.',
          "If you don't want to use a layout add this to the top of your",
          'content file:',
          '',
          '    ---',
          '    layout: false',
          '    ---',
          '',
          'Otherwise check that the directory exists or that your',
      ...
      // Make sure the layout actually exists
      if (! layout){
        var message = [
          'WOAH!'.bold,
          '',
          'haiku can not find the layout: ' + page.attributes.layout,
          '',
          'Add the layout or change the yaml front matter in the file:',
          page.path
        ].join('\n');
        log.error(message);
      }
