...
var Content = Origami.extend({
  initialize: function(attributes, haiku){
    ...
    content.set({ content: function(){
      var view = content.toView({ site: content.haiku.toView() })
        , partials = content.haiku.partials
      ;

      return Mustache.to_html(content._template, view, partials);
    }});
  },
  ...
  parse: function(content){
    ...
    if (parser === 'markdown') return markdown.parse(content);
