...
var Content = Origami.extend({
  ...
  toView: function(helpers){
    var content = this
      , view = _(this.attributes).chain().clone().value()
      , helpers = helpers || {}
      , defaultHelpers
    ;
    ...
    defaultHelpers = {
      ...
      content: function(){
        var view = content.toView({
              ...
              content: null // make sure this wont get stuck in a loop
            })
    ...
    _.extend(view, defaultHelpers, helpers);
  ...
  related: function(){
    var content = this
      ...
      , contentInCollection = this.haiku.get(collection)
      , related
    ;

    related = _(contentInCollection)
      .chain()
      .map(function(value, key){
        var intersection = _.intersection(content.get('tags'), value.tags);

        if (intersection.length > 0 && content.url() !== value.url){
          return value;
        }
