...
_.extend(Site.prototype, {
  ...
  deploy: function(){
    ...
    // console.log('options'.magenta.inverse, site.options);

    // TODO: find a good way to beam the public directory
    _.each(site.content.index(), function(page){
      if (page.attributes.draft) return;

      // console.log('beaming: '.magenta, page.buildPath());

      var destination = page.buildPath()
        , extension = path.extname(destination)
        , content = page.render()
        , req = client.put(destination, {
      ...
      req.on('response', function(res){
        if (res.statusCode === 200){
          log.info('deployed: ' + req.url);
        } else {
          log.error("Couldn't upload: " + req.url)
