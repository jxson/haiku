...
var Site = function(options) {
  ...
  this.options = _.defaults(options, {
    ...
    loglevel: 'info',
    // `options.defaultTemplateEngine` allows you to override haiku's default
    // behavior for how content is rendered. This can be set to any of the
    // supported template engines or `false`. A `false` setting will require
    // you to add the `.mustache` extension to any content you want to access
    // template variables.
...
_.extend(Site.prototype, {
  ...
  build: function() {
    ...
    mkdirp(directories.build, 0755, function(err){
      ...
      // also check the the public dir exists
