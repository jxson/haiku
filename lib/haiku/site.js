...
_.extend(Site.prototype, {
  ...
  deploy: function(){
    ...
    beam = function(_path){
      ...
      // Use a stream to avoid encoding issues
      var stream = fs.createReadStream(_path);

      client.putStream(stream, destination, function(err, res){
        if (err) throw err;

        if (res.statusCode === 200){
          log.info('deployed: ' + destination);
        } else {
      ...
      fs.readdir(name, function(err, list){
        ...
        _.each(list, function(item){
          ...
          fs.stat(_path, function(err, stats){
            ...
            if (stats.isDirectory()) readDirectory(_path);
            else beam(_path);
          });
        });
      });
